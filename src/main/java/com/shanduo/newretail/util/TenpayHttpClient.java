...
public class TenpayHttpClient {
    ...
    /*微信退款请求*/
    public String callHttpPostSSL(String url, String data) {
        CloseableHttpResponse response = null;
        CloseableHttpClient httpclient = null;
        try {
            KeyStore keyStore = KeyStore.getInstance("PKCS12");
            FileInputStream instream = new FileInputStream(new File("/www/apiclient_cert.p12"));//证书位置
            keyStore.load(instream, Config.PARTNER.toCharArray());//证书密码
            // Trust own CA and all self-signed certs
            SSLContext sslcontext = SSLContexts.custom().loadKeyMaterial(keyStore, Config.PARTNER.toCharArray()).build();
